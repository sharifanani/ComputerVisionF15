...
clear all
clc
%% Homework 6 - Where's Waldo?

%%
% Reading in the images
waldoKernel = imread('WaldoKernel.png');
waldoScene  = imread('WaldoScene.png');
%%
% Converting to L A B color space, not essential. Should work if done only
% with an rgb2gray conversion
waldoKernel = rgb2gray(waldoKernel);
waldoScene  = rgb2gray(waldoScene);
%%
% Convert images to double for processing
im1=double(waldoKernel);im2=double(waldoScene);
...
corners = [1,size(im1,1),1,size(im1,2)];
    ...
    for j = 1:size(im2,1)-size(im1,1)
        for i = 1:size(im2,2)-size(im1,2)
            ...
            patch = im2(1:size(im1,1),1:size(im1,2));
            patch = patch.*im1;%elementwise multiplication
            ...
            imResult(targetPixel(1),targetPixel(2)) = val;
